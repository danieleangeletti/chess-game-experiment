...
class Game {
  ...
  isKingUnderAttack(player, piece, destination) {
    ...
    for (let i = 0; i < boardCopy.length; i++) {
      for (let j = 0; j < boardCopy[i].length; j++) {
        if (
          boardCopy[i][j].player !== -1 &&
          boardCopy[i][j].player !== player
        ) {
  ...
  getPotentialMoves(piece) {
    ...
    switch (piece.piece) {
      case "pawn":
        potentialMoves = new Pawn(piece).captureList(this.board);
        break;
      case "rook":
        potentialMoves = new Rook(piece).captureList(this.board);
        break;
      case "knight":
        potentialMoves = new Knight(piece).captureList(this.board);
        break;
      case "bishop":
        potentialMoves = new Bishop(piece).captureList(this.board);
        break;
      case "queen":
        potentialMoves = new Queen(piece).captureList(this.board);
        break;
    }
    ...
    my_box.addEventListener("click", function () {
      ...
      if (
        ...
        if (active_box.isValidMove(game.board, clicked_box)) {
          if (
            game.isKingUnderAttack(active_box.player, active_box, clicked_box)
          ) {
